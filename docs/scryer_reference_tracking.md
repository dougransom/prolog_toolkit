...
| :--- | :--- | :--- |
| `src/parser/lexer.rs` | Token generation, escape sequences, number parsing, layout & comments, Open vs OpenCT | `prolog_lexer.pl`, `stream_annotator.pl` |
| `src/parser/parser.rs` | Operator precedence parsing, clauses, directives, expressions, syntax error handling | `prolog_parser.pl` |
| `src/parser/ast.rs` | Operator definitions, fixities, AST representations, Literals, Token enums | `prolog_token.pl`, `prolog_operator_table.pl` |
| `src/tests/parse_tokens.rs` | Unit tests for tokens, escape sequences, comments | `tests/test_prolog_lexer.pl` |
...
3. Mirror corresponding test cases into `tests/test_prolog_lexer.pl` and update toolkit logic accordingly.
